 ...
 */
var list = new List('Retain');

list.addTest('should support retained messages (MQTT-2.1.2-6, MQTT-2.1.2-7, MQTT-2.1.2-9)', function(done) {
  var t = f.t();
  var p1 = f.p();
  var p2 = f.p();
  var p3 = f.p();
...
list.addTest('should override retain for normal message forwarding (MQTT-2.1.2-10)', function(done){
  var t = f.t();
...
list.addTest('should remove retained message when payload is empty (MQTT-2.1.2-11)', function(done){
  var t = f.t();
  var p1 = f.p();
  async.waterfall([
    function(cb) {
...
list.addTest('should not override retained message on normal publish (MQTT-2.1.2-12)', function(done) {
  var t = f.t();
  var p1 = f.p();
  var p2 = f.p();
  async.waterfall([
...
/*list.addTest('should send retained messages for restored subscriptions', function(done) {
  var c = f.c();
  var t = f.t();
  var p = f.p();
  async.series([
...
list.addTest('should resend retained messages on same subscription (MQTT-3.8.4-3)', function(done) {
  var t = f.t();
  var p1 = f.p();
  var c = 0;
  async.waterfall([
...
list.addTest('should send last will if requested and keep retain (MQTT-3.1.2-15)', function(done){
